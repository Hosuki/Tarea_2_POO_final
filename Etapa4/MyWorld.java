...
public class MyWorld implements ActionListener {
   ...
   public PhysicsElement find(double x) { //encuentra los elementos dentro del marco
      for (PhysicsElement e: elements)
            if (e.contains(x,0.0)) return e;
      return null;
   } 
   ...
   public SpringAttachable findAttachableElement(double x) {
	      for (PhysicsElement e :elements)
	         if (e instanceof SpringAttachable) {
	        	 SpringAttachable ae =(SpringAttachable) e;
	            if (ae instanceof PhysicsElement)
	            if (((PhysicsElement)ae).contains(x,0.0)) return ae;
	         }
	      return null;
   }
} 